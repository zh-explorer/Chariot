...
def submit_init(max_submitters=None):
    if Context.submit_pool:
        Context.submit_pool.shutdown(wait=False)
        Context.submit_pool = None

    Context.submit_pool = concurrent.futures.ThreadPoolExecutor(max_submitters)
...
class FlagSubmitter(object):
    def __init__(self, flag_id: int, notify_queue: queue.Queue):
        if Context.submit_pool is None:
            submit_init(Context.max_submitters)
    ...
    def run_finish(self, result):
        ...
        flag.submit_status = FlagStatus[result[0]]
        ...
        session.commit()
        session.close()
        self.notify_queue.put(self.flag_id)
